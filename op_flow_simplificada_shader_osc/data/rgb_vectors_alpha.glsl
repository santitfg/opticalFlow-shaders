...
/*
    color.r = texture2D(cam,st+vec2(vectores[0],vectores[1])*0.75+vec2(vectores[4],vectores[5])*0.25).r;
    color.g = texture2D(cam,st+vec2(vectores[2],vectores[3])*0.75+vec2(vectores[0],vectores[1])*0.25).g;
    color.b = texture2D(cam,st+vec2(vectores[4],vectores[5])*0.75+vec2(vectores[2],vectores[3])*0.25).b;
*/
...
//probar luego desfases
  ...
  //gl_FragColor = vec4(color, color.g);

  gl_FragColor = vec4(color, (color.g+color.r+color.b)/3.);
