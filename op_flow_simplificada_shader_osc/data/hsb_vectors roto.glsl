...
vec2 desfaceB=vec2(0,0);
    color.r = hsb2rgb(texture2D(cam,fract(st+vec2(vectores[0],vectores[1]))).rgb).r;
    color.g = hsb2rgb(texture2D(cam,fract(st+vec2(vectores[2],vectores[3]))).rgb).g;
    color.b = hsb2rgb(texture2D(cam,fract(st+vec2(vectores[4],vectores[5]))).rgb).b;
