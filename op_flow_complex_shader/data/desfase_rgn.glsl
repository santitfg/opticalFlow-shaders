...
void main( void ) { 
    ...
    for(int  i= 0; i< vectores.length() ; i+=4){
        ...
        color.r = texture2D(cam,st+float(vec2(vectores[i],vectores[i+1])==id)*0.75*vec2(vectores[i+2],vectores[i+3])).r;
        color.g = texture2D(cam,st+float(vec2(vectores[i],vectores[i+1])==id)*0.5*vec2(vectores[i+2],vectores[i+3])).g;
        color.b = texture2D(cam,st+float(vec2(vectores[i],vectores[i+1])==id)*0.1*vec2(vectores[i+2],vectores[i+3])).b;
